...
function isFileUpToDate(node, item, callback) {
    ...
    const prefix = path.join(node.path, 'src')

    const postfix = item.path.substring(prefix.length, item.path.length)

    const destination = path.join(node.path, 'lib', postfix)
        ...
        if (!destStats.isFile()) {
            console.log(
                chalk.yellow('Warning') +
                    'Is upToDate unexpected result for ' +
                    destination,
            )
            return callback(null, false)
        }
