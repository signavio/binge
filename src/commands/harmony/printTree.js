...
export default function(
    ...
    const widthHeader =
        calcWidth(
            dependencyStatus.map(
                ({ name, version }) => `${name} ${version || 'failed'}`
            )
        ) + 2
    const widthCol1 =
        calcWidth(dependencyPointers.map(pointer => pointer.nodeName)) + 4
    const widthCol2 = calcWidth(
        dependencyPointers.map(pointer => pointer.version || 'failed')
    )
                ...
                .forEach(({ nodeName, version }) => {
                    console.log(
                        `${pad(nodeName + ' -> ', widthCol1)}` +
                            `${pad(
                                widthTotal - widthCol1 - version.length,
                                ''
                            )}` +
